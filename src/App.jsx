...
async function applySettings(app, profileId, { langStates, minTier, maxTier, hdr, maxSize }) {
  ...
  let lowId = null, lowTier = Infinity, highId = null, highTier = -1
  for (const it of p.items) {
    ...
    if (allowed && tier < lowTier) { lowTier = tier; lowId = id }
...
function Seg({ options, value, onChange, small }) {
  return (
    <div className={'seg' + (small ? ' small' : '')}>
      {options.map(o => <button key={o.id} className={'seg-btn ' + o.id + (value === o.id ? ' on' : '')} onClick={() => onChange(o.id)} title={o.hint || ''}>{o.label}</button>)}
    </div>
  )
}
...
function Panel({ app }) {
  ...
  useEffect(() => { (async () => {
    try { const list = await getProfiles(app); setProfiles(list); setSel(list.find(p => p.id === 4)?.id ?? list[0]?.id) }
    catch (e) { setStatus({ text: 'keine Verbindung (Stack/Docker an?)', kind: 'err' }) }
  })() }, [])
  ...
  return (
    <section className="card" id={app.anchor}>
      ...
      <div className="langs">
        {LANGUAGES.map(l => (
          <div className="lang-row" key={l.id}><span className="lang-name"><span className="flag">{l.flag}</span>{l.label}</span><Seg small options={STATES} value={langStates[l.id]} onChange={st => setLang(l.id, st)} /></div>
        ))}
      </div>
      <h3>Qualität</h3>
...
function suggestFix(reasons) {
  ...
  if (txt.includes('custom format') && txt.includes('minimum')) return { why: 'Deine Sprach-Pflicht ist zu streng – kein Release erfüllt sie.', fix: 'Im „Einstellungen"-Tab die Sprache (z. B. Deutsch) auf „Optional" statt „Pflicht" stellen.' }
  if (txt.includes('quality') && (txt.includes('not wanted') || txt.includes('rejected') || txt.includes('cutoff'))) {
    ...
    return { why: 'Die gefundenen Releases passen nicht zur erlaubten Qualität.', fix: 'Im „Einstellungen"-Tab die Qualitäts-Spanne erweitern (z. B. „bis 4K“).' }
  }
  if (txt.includes('size')) return { why: 'Die Releases sind größer als deine Max-Größe.', fix: 'Im „Einstellungen"-Tab die „Max. Dateigröße" erhöhen oder auf „Aus".' }
  if (!reasons.length) return { why: 'Es wurden keine Releases gefunden (gibt es evtl. nicht auf den öffentlichen Indexern).', fix: 'Im „Suchen"-Tab manuell prüfen, oder einen deutschen Tracker hinzufügen.' }
...
function Status() {
  ...
  const load = async () => {
    try {
      ...
      const [rq, sq, rm, sm, qbt] = await Promise.all([
        fetch('/radarr/api/v3/queue?pageSize=100', h(RADARR_KEY)).then(r => r.json()).catch(() => ({ records: [] })),
        fetch('/sonarr/api/v3/queue?pageSize=100', h(SONARR_KEY)).then(r => r.json()).catch(() => ({ records: [] })),
        fetch('/radarr/api/v3/wanted/missing?pageSize=60', h(RADARR_KEY)).then(r => r.json()).catch(() => ({ records: [], totalRecords: 0 })),
        fetch('/sonarr/api/v3/wanted/missing?pageSize=60', h(SONARR_KEY)).then(r => r.json()).catch(() => ({ records: [], totalRecords: 0 })),
  ...
  const why = async (appId, kind, item) => {
    setModal({ title: item.title || `${item.series?.title} S${item.seasonNumber}E${item.episodeNumber}`, loading: true })
    try {
      ...
      const reasons = [...new Set(rel.flatMap(r => r.rejections || []))].slice(0, 4)
      const s = suggestFix(reasons)
      setModal({ title: item.title || `${item.series?.title} S${String(item.seasonNumber).padStart(2,'0')}E${String(item.episodeNumber).padStart(2,'0')}`, loading: false, found: rel.length, accepted: accepted.length, reasons, ...s, appId, item, kind })
  ...
  const searchNow = async (appId, item) => {
    try {
      ...
      await api(app, '/api/v3/command', { method: 'POST', body })
      setModal(m => ({ ...m, searched: true }))
  ...
  ;(d?.queue || []).forEach(q => {
    if ((d.torrents || []).some(t => t.name === q.title || t.name.replace(/\.\w+$/, '') === q.title)) return
    rows.push({ key: 'q' + q.title, label: q.kind + ' ' + q.title, prog: q.size > 0 ? (q.size - q.sizeleft) / q.size : 0, state: q.status + (q.trackedDownloadState ? ' · ' + q.trackedDownloadState : ''), size: q.size, sizeleft: q.sizeleft })
  })
        ...
        <section className="card status-card">
          ...
          {rows.map(r => (
            <div className="dl-row" key={r.key}>
              ...
              <div className="bar"><div className="bar-fill" style={{ width: Math.round(r.prog * 100) + '%' }} /></div>
        ...
        <section className="card status-card warn">
          ...
          <div className="miss-grid">
            <div>
              ...
              {(d.missR.records || []).slice(0, 12).map(m => (
                <div className="miss-item act" key={m.id}><span>{m.title} {m.year ? '(' + m.year + ')' : ''}</span><button className="why-btn" onClick={() => why('radarr', '🎬', m)}>Warum?</button></div>
              ))}
            ...
            <div>
              ...
              {(d.missS.records || []).slice(0, 12).map(m => (
                <div className="miss-item act" key={m.id}><span>{m.series?.title || ''} S{String(m.seasonNumber).padStart(2, '0')}E{String(m.episodeNumber).padStart(2, '0')}</span><button className="why-btn" onClick={() => why('sonarr', '📺', m)}>Warum?</button></div>
              ))}
              {(d.missS.records || []).length === 0 && <div className="miss-item empty">— nichts —</div>}
            </div>
          </div>
        </section>
      </>}
...
function SearchTab() {
  ...
  useEffect(() => { (async () => {
    try {
      ...
      const list = await api(a, appId === 'radarr' ? '/api/v3/movie' : '/api/v3/series')
      list.sort((x, y) => (x.sortTitle || x.title).localeCompare(y.sortTitle || y.title))
      setItems(list); setSelId(list[0]?.id ?? ''); setResults(null); setMsg('')
  ...
  const doSearch = async () => {
    ...
    try {
      const a = appById(appId)
      const q = appId === 'radarr' ? `movieId=${selId}` : `seriesId=${selId}` + (season ? `&seasonNumber=${season}` : '')
      const rel = await api(a, `/api/v3/release?${q}`)
      rel.sort((x, y) => (Number(!!x.rejected) - Number(!!y.rejected)) || (tierOf(y.quality?.quality?.name || '') - tierOf(x.quality?.quality?.name || '')) || ((y.seeders || 0) - (x.seeders || 0)))
      setResults(rel); setMsg(rel.length ? '' : 'Keine Treffer.')
    } catch (e) { setMsg('Fehler: ' + e.message) }
    setLoading(false)
  }
  const grab = async (r) => {
    try { await api(appById(appId), '/api/v3/release', { method: 'POST', body: JSON.stringify({ guid: r.guid, indexerId: r.indexerId }) }); setMsg('„' + r.title + '" wird geladen! → Downloads-Tab.') }
    catch (e) { setMsg('Laden fehlgeschlagen: ' + e.message) }
  }
        ...
        <section className="card">
          ...
          <div className="res-list">
            {results.slice(0, 40).map((r, i) => {
              const hdr = detectHDR(r.title)
              const langs = (r.languages || []).map(l => l.name).filter(n => n && n !== 'Unknown').join(', ')
              return (
                <div className={'res-row' + (r.rejected ? ' rej' : '')} key={i}>
                  <div className="res-main"><div className="res-title">{r.title}</div>
                    <div className="res-tags">
                      <span className="tag q">{r.quality?.quality?.name || '?'}</span><span className="tag">{fmtBytes(r.size)}</span>
                      <span className={'tag ' + (hdr === 'SDR' ? '' : 'hdr')}>{hdr}</span>{langs && <span className="tag">{langs}</span>}
                      <span className="tag">⬆ {r.seeders ?? '?'}</span><span className="tag">{r.indexer}</span>
                      {r.rejected && <span className="tag bad" title={(r.rejections || []).join('; ')}>abgelehnt</span>}
                    </div>
                  </div>
          ...
          <p className="hint">Sortiert: passende zuerst, dann höchste Qualität, dann meiste Seeder. „Abgelehnt" = passt nicht zur Regel, kann trotzdem geladen werden.</p>
        </section>
      )}
    </div>
  )
...
export default function App() {
  ...
  return (
    <div className="app">
      ...
      <footer>
        <div><b>Tipp:</b> Standard für beste 4K ohne Riesendateien: <i>HDR bevorzugt</i> + <i>Max 25 GB</i>. Manuell aussuchen im <i>Suchen</i>-Tab.</div>
